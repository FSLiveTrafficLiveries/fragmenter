import EventEmitter$3 from 'events';
...
import util$1, { promisify as promisify$1 } from 'util';
...
import require$$0$3 from 'async_hooks';
import crypto from 'crypto';
import require$$0$4 from 'worker_threads';

var FragmenterOperation;
...
})(FragmenterOperation || (FragmenterOperation = {}));

class FragmenterContext extends EventEmitter$3 {
    constructor(options, signal) {
        // eslint-disable-next-line constructor-super
        super();
        this.signal = signal;
        this.phase = { op: FragmenterOperation.NotStarted };
...
if (!fs$1[gracefulQueue]) {
  ...
  if (/\bgfs4\b/i.test(process.env.NODE_DEBUG || '')) {
    process.on('exit', function() {
      ...
      assert.equal(fs$1[gracefulQueue].length, 0);
    });
  }
}
...
function hasMillisResSync () {
  ...
  tmpfile = path.join(os__default.tmpdir(), tmpfile);
...
function read(root, filter, files, prefix) {
  ...
  if (!fs$1.existsSync(dir)) return files
...
var Writable$2 = require$$0$1.Writable;
...
util$1.inherits(DelayedStream, Stream$2);
...
// make it a Stream
util$1.inherits(FormData$2, combined_stream);
...
class FragmenterUpdateChecker extends EventEmitter$3 {
    /**
...
})(FragmenterErrorCode || (FragmenterErrorCode = {}));
class FragmenterError extends Error {
    ...
    static parseFromMessage(message) {
        ...
        const code = FragmenterErrorCode[codeString];
...
class StreamDownloader extends EventEmitter$3 {
...
class FileDownloader extends EventEmitter$3 {
    ...
    download(dest) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (serverSupportsRanges) {
                ...
                if (loadedBytes < fileSize) {
                    if (!ret.error) {
                        ret.error = FragmenterError.create(FragmenterErrorCode.MaxModuleRetries, 'File not entirely downloaded - max number of download resumes reached or unrecoverable error detected');
                    }
                }
            }
            else {
                ...
                if (loadedBytes < fileSize) {
                    if (!ret.error) {
                        ret.error = FragmenterError.create(FragmenterErrorCode.MaxModuleRetries, 'File not entirely downloaded - max number of download restarts reached');
                    }
                }
            }
...
class ModuleDownloader extends EventEmitter$3 {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                try {
                    ...
                    yield promisify$1(lib$1.rm)(filePath);
                }
                catch (e) {
                    // noop
                }
    ...
    mergeModuleFileParts(destDir) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const numParts = this.module.splitFileCount;
            for (let i = 0; i < numParts; i++) {
                const completeModuleFileWriteStream = lib$1.createWriteStream(path.join(destDir, `${this.module.name}.zip`), { flags: 'a' });
                const partIndexString = (i + 1).toString()
                    .padStart(numParts.toString().length, '0');
                const filePath = path.join(destDir, `${this.module.name}.zip.fg-tmp${partIndexString}`);
                try {
...
function pumpFileDataReadStream(self, entry, readStream) {
  ...
  var compressor = entry.compress ? new zlib.DeflateRaw() : new PassThrough$2();
...
util$1.inherits(ByteCounter, Transform$1);
...
util$1.inherits(Crc32Watcher, Transform$1);
...
function unlink(path) {
    return util$1.promisify(fs$1.unlink)(path);
}
exports.unlink = unlink;
function mkdir(path, mode) {
    return util$1.promisify(fs$1.mkdir)(path, mode);
}
exports.mkdir = mkdir;
function realpath(path) {
    return util$1.promisify(fs$1.realpath)(path);
}
exports.realpath = realpath;
function stat(path) {
    return util$1.promisify(fs$1.stat)(path);
}
exports.stat = stat;
function lstat(path) {
    return util$1.promisify(fs$1.lstat)(path);
}
exports.lstat = lstat;
function chmod(path, mode) {
    return util$1.promisify(fs$1.chmod)(path, mode);
}
exports.chmod = chmod;
function readdir(path) {
    return util$1.promisify(fs$1.readdir)(path);
}
exports.readdir = readdir;
function access(path, mode) {
    return util$1.promisify(fs$1.access)(path, mode);
}
exports.access = access;
function rmdir(path) {
    return util$1.promisify(fs$1.rmdir)(path);
}
exports.rmdir = rmdir;
function symlink(target, path, type) {
    return util$1.promisify(fs$1.symlink)(target, path, type);
}
exports.symlink = symlink;
function readlink(path) {
    return util$1.promisify(fs$1.readlink)(path);
}
...
class Zip extends cancelable.Cancelable {
    ...
    async archive(zipFile) {
        ...
        this.token = token;
        this.isPipe = false;
        await fs.ensureFolder(path.dirname(zipFile));
        ...
        return new Promise(async (c, e) => {
            ...
            if (!token.isCancelled) {
                this.zipStream = (0, fs$1.createWriteStream)(zipFile);
    ...
    addFileStream(zip, file, metadataPath, token) {
        return new Promise((c, e) => {
            const fileStream = (0, fs$1.createReadStream)(file.path);
...
var PassThrough$1 = require$$0$1.PassThrough;

var EventEmitter$1 = EventEmitter$3.EventEmitter;
...
var EventEmitter = EventEmitter$3.EventEmitter;
        ...
        return new Promise(async (c, e) => {
            try {
                ...
                if (isSymlink && !this.symlinkToFile()) {
                    ...
                    fileStream = (0, fs$1.createWriteStream)(filePath, { mode });
...
class ModuleDecompressor extends EventEmitter$3 {
    ...
    decompress(filePath, destDir) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (actualCrc !== expectedCrc) {
                throw FragmenterError.create(FragmenterErrorCode.ModuleCrcMismatch, `expected: ${expectedCrc.substring(0, 8)}, read: ${actualCrc.substring(0, 8)}`);
            }
...
class FragmenterInstaller extends EventEmitter$3 {
    constructor(ctx, baseUrl, destDir, options) {
        ...
        this.options = Object.assign({ temporaryDirectory: path.join(os.tmpdir(), `${DEFAULT_TEMP_DIRECTORY_PREFIX}-${(Math.random() * 1000000).toFixed(0)}`), maxModuleRetries: 5, forceFreshInstall: false, disableFallbackToFull: false }, options);
    }
    install() {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                try {
                    yield lib$1.access(path.join(this.options.temporaryDirectory, 'restore'));
    ...
    downloadAndInstallModule(module, moduleIndex, fullModuleHash, move = true) {
        return __awaiter(this, void 0, void 0, function* () {
                ...
                try {
                    ...
                    if (FragmenterError.isFragmenterError(e)) {
                        ...
                        if (UnrecoverableErrors.includes(fragmenterError.code)) {
                            this.ctx.logError(`[FragmenterInstaller] Unrecoverable error (${FragmenterErrorCode[fragmenterError.code]}) encountered during module '${module.name}' download and install`);
            ...
            throw FragmenterError.create(FragmenterErrorCode.MaxModuleRetries, `max number of retries reached for module '${module.name}'`);
        });
    }
    ...
    cleanupTempModuleFiles(module) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                if (moduleZipExists) {
                    yield promisify$1(lib$1.rm)(moduleZipPath);
                }
                if (moduleExtractDirExists) {
                    yield promisify$1(lib$1.rm)(moduleExtractDir, { recursive: true });
                }
            }
            catch (e) {
    ...
    moveOverExtractedFiles(module) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (const sd of sourceDirs) {
                ...
                const destModuleDir = path.join(this.destDir, sd);
                yield this.moveOverModuleFiles(module, absoluteSourceDir, destModuleDir);
            }
        });
    }
    ...
    ensureTempDirRemoved() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (tempDirExists) {
                try {
                    yield promisify$1(lib$1.rm)(this.options.temporaryDirectory, { recursive: true });
                }
                catch (e) {
    ...
    ensureDestDirIsEmpty() {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                yield promisify$1(lib$1.rm)(this.destDir, { recursive: true });
            }
            catch (e) {
                this.emit('error', '[FragmenterInstaller] Error while emptying dest directory');
            }
            try {
                yield promisify$1(lib$1.mkdir)(this.destDir);
            }
            catch (e) {
...
ret.nodeSupportsAsyncResource = ret.isNode && (function() {
    ...
    try {
        var res = require$$0$3.AsyncResource;
...
var AsyncResource = util$1.isNode && util$1.nodeSupportsAsyncResource ?
    require$$0$3.AsyncResource : null;
...
SplitFile.prototype.splitFile = function (file, parts, dest) {
  ...
  return bluebird_1.promisify(fs$1.stat)(file).then(function (stat) {
    if (!stat.isFile) {
...
SplitFile.prototype.__splitFile = function (file, partInfo, dest) {
  ...
  return bluebird_1.mapSeries(partInfo, function (info) {
    return new bluebird_1(function (resolve, reject) {
      // Open up a reader
      var reader = fs$1.createReadStream(file, {
        encoding: null,
        start: info.start,
        end: info.end - 1,
      });
      ...
      const outputFile = (filename) => {
        const writer = fs$1.createWriteStream(filename);
...
if (Worker === undefined) {
	hasha.fromFile = async (filePath, options) => hasha.fromStream(fs$1.createReadStream(filePath), options);
...
hasha.fromFileSync = (filePath, options) => hasha(fs$1.readFileSync(filePath), options);
...
function pack(buildManifest) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const zip = (sourcePath, zipDest) => __awaiter(this, void 0, void 0, function* () {
            ...
            const allFiles = fsReaddirRecursive(sourcePath).map((i) => path.resolve(sourcePath, i));
            const crcInfo = {
                hash: yield generateHashFromPaths(allFiles, sourcePath),
            };
            yield lib$1.writeJSON(path.join(sourcePath, SINGLE_MODULE_MANIFEST), crcInfo);
            ...
            for (const file of allFiles) {
                ...
                yield zip.addFile(file, relativePath);
            }
            ...
            const sizeUncompressed = allFiles.reduce((accu, filePath) => accu + lib$1.statSync(filePath).size, 0);
                    ...
                    for (const sourceA of sourceDirsA) {
                        for (const sourceB of sourceDirsB) {
                            ...
                            if (!pathDiff.startsWith("..")) {
                                throw new Error(`Module '${moduleA.name}' directory '${sourceA}' contains module '${moduleB.name}' directory '${sourceB}'. Modules within modules are not supported yet!`);
                            }
                        }
                    }
                }
        ...
        try {
            ...
            ] = yield zip(tempDir, path.join(buildManifest.outDir, FULL_FILE));
            // Zip Modules
            ...
            for (const module of buildManifest.modules) {
                ...
                const allFiles = sourceDirs.flatMap(dir => fsReaddirRecursive(path.join(tempDir, dir)).map(f => path.resolve(tempDir, dir, f)));
                const moduleHash = yield generateHashFromPaths(allFiles, tempDir);
                ...
                for (const dir of sourceDirs) {
                    ...
                    const filesInDir = fsReaddirRecursive(sourcePath).map((i) => path.resolve(sourcePath, i));
                    sizeUncompressed += filesInDir.reduce((accu, filePath) => accu + lib$1.statSync(filePath).size, 0);
                    // Add the directory and its contents to maintain structure
                    yield zip.addFile(sourcePath, dirName);
                    for (const file of filesInDir) {
                        const relativePath = path.relative(sourcePath, file);
                        yield zip.addFile(file, path.join(dirName, relativePath));
                    }
                }
                // Archive the zip
                yield zip.archive(zipDest);
                ...
                // Store in manifest
                distributionManifest.modules.push(Object.assign(Object.assign({}, module), { hash: moduleHash, splitFileCount, completeFileSize: zipStat.size, completeFileSizeUncompressed: sizeUncompressed }));
            }
            // Zip the rest
            ...
            yield lib$1.writeJSON(path.join(buildManifest.outDir, MODULES_MANIFEST), distributionManifest);
...
export { FragmenterContext, FragmenterError, FragmenterErrorCode, FragmenterInstaller, FragmenterOperation, FragmenterUpdateChecker, UnrecoverableErrors, getCurrentInstall, pack };
